...
export async function initCommit(path: string, packageManager: string) {
  return new Promise((resolve, reject) => {
    exec(
      `ls && cd ${path} && ${packageManager} install && git status && git add . && git status && git commit -m 'init commit generated by starter.dev CLI' && git status && ls && cd ../ && ls`,
      (err, stdout) => {
        if (err) {
          console.log('err', err);
          reject(err);
